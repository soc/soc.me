...
---
  ...
  - `from`    : if argument is adapted, converted, etc.
              ...
              e. g. `Person.from(string)`, `Person.fromString(string)`, `Person.from[String](string)`
  - `to`      : implies a conversion of some sort, e. g. `array.toList`, `int.toDouble`, `array.to(List)`, `int.to(Double)`
  - `as`      : implies a reinterpretation/wrapping of a verbatim value, e. g. `int.asDouble`, `int.as(Double)`
