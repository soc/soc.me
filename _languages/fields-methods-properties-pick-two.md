...
_**TL;DR:** Properties are a hack employed to retrofit "nice" syntax into languages that already shipped with fields and methods. Instead, design the language to deliver the same (or more) benefits with fields!_
...
and changing a getter/setter pair to a property is neither.
...
because the IDE can use different colors to mark fields (keyword `let`) and methods (keyword `fun`).
